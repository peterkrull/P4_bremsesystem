...
struct k_t *pRadio, *pDrive;
struct k_t *TimeSemRadio;
...
float pwm_kicker = (1/(float)freq)*1000;

unsigned long lastMessage = 0;    //0 Stand still, 1 drive mode, 2 break mode

const int pin_stopped = 10;
const int pin_drive = 11;
const int pin_reverse = 12;
...
float check_input(){
  float calculated = ((float)(analogRead(pin_pot))/511.5)-1;
  return calculated;
}
...
void pwm_out_v2(float duty){
  ...
  if (curr_time >= prev_lo + pwm_kicker){ // PWM signal HIGH
    prev_lo = curr_time;
    hightime = (int)((1/(float)freq)*(1-abs(duty))*1000);
    prev_hi = curr_time + pwm_kicker - hightime;
    if (duty < 0.02 && duty > -0.02){ // If duty cycle is low => considered 0
      digitalWrite(pin_drive,LOW);
      digitalWrite(pin_reverse,LOW);
    } else if (duty >= 0.02) {
...
void scriptlet(unsigned long timeunder, float wanted, boolean linear, boolean stopping = false){
    ...
    while (millis() < timeunder + start_timer && state != 3 && state != 4 && state != 5 && stopped == 0){
      if (stopping == true ){
...
void drive_script_v1(){
  //time_factor = 1; // Slower accel
  //test_speed = 0.2;
  scriptlet(100,0,false,false); // 100 ms off-time
  scriptlet(3000,test_speed,true, false); // 3 seconds of accel
  //time_factor = 1; // Faster brake
  //test_speed = 0.2;
  scriptlet(100,0,false,false); // 250 ms off-time
  scriptlet(3000,-test_speed,true, true); // Brake for 3 seconds or until stopped
  scriptlet(250,0,false,false); // Force stop-state
}


void drive_script_v2(){
  time_factor = 1; // Slower accel
  test_speed = 1;
  scriptlet(100,0,false,false); // 100 ms off-time
  scriptlet(3000,test_speed,true, false); // 3 seconds of accel
  ...
  test_speed = 1;
  scriptlet(100,0,false,false); // 250 ms off-time
  scriptlet(2000,-test_speed,true, false); // Brake for 3 seconds or until stopped
  scriptlet(250,0,false,false); // Force stop-state
}
